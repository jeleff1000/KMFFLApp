...
class GaviStatViewer(WeeklyMatchupDataViewer):
    ...
    def display(self):
        ...
        with col1:
            ...
            selected_manager = st.selectbox("Select Manager", managers, key="gavi_stat_manager_dropdown", help="Select the manager for the simulation")
        with col2:
        ...
        aggregate_toggle = st.toggle("Aggregate All Years", value=False, key="gavi_stat_aggregate_toggle")

        # Calculate xWins, xLosses, and delta for each Manager and year
        def calculate_xwins_xlosses(df):
            df['xWins'] = df['teams_beat_this_week'] / 9
        ...
        # Check if 'teams_beat_this_week' exists before dropping
        if 'teams_beat_this_week' in result_df.columns:
