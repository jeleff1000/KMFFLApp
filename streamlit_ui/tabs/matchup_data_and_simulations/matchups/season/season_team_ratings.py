...
class SeasonTeamRatingsViewer:
    ...
    def __init__(self, df: pd.DataFrame) -> None:
        ...
        # Columns not to show at season level
        self.hidden_cols: List[str] = ["Opponent Week", "OpponentYear", "week", "opponent"]
        ...
        self.secondary_order: List[str] = [
            ...
            "avg_seed", "Final Playoff Seed",  # prefer source column
            # Probabilities averaged across the season
            "p_playoffs", "p_bye", "exp_final_wins", "p_semis", "p_final", "p_champ",
            # Shuffle/finals taken from last week of season
            ...
            # Keep x1_seed available if present (not shown unless listed)
            # "x1_seed",
        ]
    ...
    def _aggregate_by_season(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        min_cols = [c for c in ["Losing Streak"] if c in present]  # min per requirement

        # Last-week values
        last_cols = [c for c in [
            "Final Playoff Seed",  # prefer source column
            "x1_seed",             # fallback if needed, not shown by default
            "shuffle_avg_wins",
        ...
        # If Final Playoff Seed missing but x1_seed exists, create it from x1_seed
    ...
    def display(self, prefix: str = "season_team_ratings") -> None:
        st.subheader("Season Team Ratings")
        ...
        # Columns to show: leading, then secondary only
