...
class SeasonMatchupStatsViewer:
    ...
    def display_weekly_graphs(self, prefix=""):
        ...
        required_cols = {'Manager', 'week', 'team_points', 'year', 'opponent', 'Final Playoff Seed'}
        if self.df is not None and required_cols.issubset(self.df.columns) and not self.df.empty:
            ...
            with col1:
                ...
                selected_managers = st.multiselect("Select Manager(s)", managers, default=[], key=f"{prefix}_managers_graph")
                if not selected_managers:
                    selected_managers = managers
            with col2:
                opponents = sorted(self.df['opponent'].unique().tolist())
                selected_opponents = st.multiselect("Select Opponent(s)", opponents, default=[], key=f"{prefix}_opponents_graph")
                if not selected_opponents:
                    selected_opponents = opponents
            with col3:
                years = sorted(self.df['year'].astype(int).unique().tolist())
                selected_years = st.multiselect("Select Year(s)", years, default=[], key=f"{prefix}_years_graph")
                if not selected_years:
                    selected_years = years
            with col4:
                seeds = sorted(self.df['Final Playoff Seed'].dropna().unique().tolist())
                selected_seeds = st.multiselect("Select Final Playoff Seed(s)", seeds, default=[], key=f"{prefix}_seeds_graph")
            ...
            if x_axis_week:
                ...
                week_avg_all = self.df.groupby('week')['team_points'].mean().reset_index()
                week_avg_all['team_points'] = week_avg_all['team_points'].round(2)
                avg_line_all = alt.Chart(week_avg_all).mark_line(strokeDash=[5, 5], size=5, color='black').encode(
                    x=alt.X('week:Q', axis=x_axis),
                    y=alt.Y('team_points:Q', scale=alt.Scale(domain=[50, 220]))
                )
                # Filtered average
                week_avg_filtered = df_sorted.groupby('week')['team_points'].mean().reset_index()
                week_avg_filtered['team_points'] = week_avg_filtered['team_points'].round(2)
                avg_line_filtered = alt.Chart(week_avg_filtered).mark_line(strokeDash=[5, 5], size=5, color='red').encode(
                    x=alt.X('week:Q', axis=x_axis),
                    y=alt.Y('team_points:Q', scale=alt.Scale(domain=[50, 220]))
                )
                ...
                league_avg_all = df_all.groupby(['year', 'Cumulative Week']).agg({'League Cumulative Avg': 'last'}).reset_index()
                cumulative_line_all = alt.Chart(league_avg_all).mark_line(strokeDash=[5, 5], size=5, color='black').encode(
                ...
                league_avg_filtered = df_sorted.groupby(['year', 'Cumulative Week']).agg({'League Cumulative Avg': 'last'}).reset_index()
                cumulative_line_filtered = alt.Chart(league_avg_filtered).mark_line(strokeDash=[5, 5], size=5, color='red').encode(
                    x=alt.X('Cumulative Week:O', axis=x_axis),
                    y=alt.Y('League Cumulative Avg:Q', scale=alt.Scale(domain=[50, 220])),
                    detail='year:N'
                )
                ...
                chart = points + cumulative_line_all + cumulative_line_filtered + year_rules + year_labels

            chart = chart.properties(width=800, height=400)
            st.altair_chart(chart, use_container_width=True)
        else:
            st.write("Required columns (`Manager`, `week`, `team_points`, `year`, `opponent`, `Final Playoff Seed`) are missing or DataFrame is empty.")
