...
# -------------------------
# Per-week blended metrics (on-demand compute)
# -------------------------
@st.cache_data(show_spinner=False)
def compute_weekly_timeseries(df_all, season):
    """
    Returns a long DataFrame with columns:
    [year, week, Manager, P_Playoffs, P_Bye, Exp_Final_Wins, P_Semis, P_Final, P_Champ]
    using the blended method (simulation + historical kernels) per week.
    """
    df = df_all[(df_all["is_consolation"] == 0)].copy()
    reg = df[(df["year"] == season) & (df["is_playoffs"] == 0)].copy()
    if reg.empty:
        return pd.DataFrame()

    weeks = sorted(reg["week"].unique())
    n_teams = reg["Manager"].nunique()
    ...
    rows = []
    for week in weeks:
        played_raw = reg[reg["week"] <= week].copy()
        future_canon = canonicalize(reg[reg["week"] > week].copy())
        ...
        # Sims for remaining season / playoffs
        ...
        if not future_canon.empty and week not in playoff_weeks:
            for _ in range(N_SIMS):
                ...
                playoff_r2.extend(r2); playoff_r3.extend(r3)
                if champ is not None: champions.append(champ)
                sims.append(seeds)
        else:
            wins_to_date, pts_to_date = wins_points_to_date(played_raw)
            fixed_seeds = rank_and_seed(wins_to_date, pts_to_date, PLAYOFF_SLOTS, BYE_SLOTS, played_raw=played_raw)
            for _ in range(N_SIMS):
                sim_seeds = fixed_seeds.copy()
                r2, r3, champ = simulate_playoff_bracket(
                    sim_seeds, playoff_schedule, rng, BYE_SLOTS,
                    current_week=week, mu_hat=mu_hat, sigma_hat=sigma_hat, samples_by_team=samples_by_team
                )
                playoff_r2.extend(r2); playoff_r3.extend(r3)
                if champ is not None: champions.append(champ)
        ...
        odds["P_Semis"] = odds.index.map(lambda m: playoff_r2.count(m) / N_SIMS)
        odds["P_Final"] = odds.index.map(lambda m: playoff_r3.count(m) / N_SIMS)
        odds["P_Champ"] = odds.index.map(lambda m: champions.count(m) / N_SIMS)
        avg_seed = tall.groupby("Manager")["seed"].mean().rename("Avg_Seed")
        ...
        sim_seed_dist = (
            ...
            .div(len(sims))
        ).reindex(columns=list(range(1, n_teams+1)), fill_value=0.0)
        hist_seed_dist = empirical_kernel_seed_dist(
            played_raw, week, history_snapshots, n_teams,
            h_W=0.9, h_L=0.9, h_PF=15.0, h_week=0.9,
            prior_strength=3.0
        ).reindex(index=sim_seed_dist.index, columns=list(range(1, n_teams+1)), fill_value=0.0)
        ...
        odds["P_Playoffs"] = odds.apply(
            lambda r: sim_w * r["P_Playoffs"] + base_w * baseline_playoff.get(r.name, p0),
            axis=1
        )
        blended_seed = (sim_w * sim_seed_dist + base_w * hist_seed_dist)
        odds["P_Bye"] = blended_seed[1] + blended_seed[2]

        # normalize to league totals
        if odds["P_Playoffs"].sum() > 0:
            odds["P_Playoffs"] *= (PLAYOFF_SLOTS / odds["P_Playoffs"].sum())
        if odds["P_Bye"].sum() > 0:
            odds["P_Bye"] *= (BYE_SLOTS / odds["P_Bye"].sum())

        # percentages for plotting
        pct_cols = ["P_Playoffs", "P_Bye", "P_Semis", "P_Final", "P_Champ"]
        for c in pct_cols:
            odds[c] = (100.0 * odds[c]).round(4)
        odds["Exp_Final_Wins"] = odds["Exp_Final_Wins"].round(4)

        for _, r in odds.reset_index().iterrows():
            rows.append({
                "year": season,
                "week": week,
                "Manager": r["Manager"],
                ...
                "P_Bye": r["P_Bye"],
                "Exp_Final_Wins": r["Exp_Final_Wins"],
                "P_Semis": r["P_Semis"],
                "P_Final": r["P_Final"],
                "P_Champ": r["P_Champ"],
            })
...
# -------------------------
# New Viewer: line chart over weeks (on-demand)
# -------------------------
METRIC_LABELS = {
...
class PlayoffOddsViewer:
    ...
    def display(self):
        ...
        go_clicked = col3.button("Go")

        # Session state to avoid auto-initializing and to persist results
        if "ts_data" not in st.session_state:
            st.session_state.ts_data = None
            st.session_state.ts_season = None

        # Compute only when Go is clicked
        if go_clicked:
            with st.spinner("Computing weekly odds..."):
                ts = compute_weekly_timeseries(df, season)
            st.session_state.ts_data = ts
            st.session_state.ts_season = season

        # Clear results if user changes year and wants to reset
        clear = st.button("Clear")
        if clear:
            st.session_state.ts_data = None
            st.session_state.ts_season = None

        # If we have no computed data yet, stop here
        if st.session_state.ts_data is None or st.session_state.ts_season != season:
            st.info("Select a Year & Metric, then press **Go** to generate the chart.")
            return

        timeseries = st.session_state.ts_data
        if timeseries.empty:
            st.info("No regular season data for the selected year.")
            return
        ...
        selected_mgrs = st.multiselect(
            "Managers",
            managers,
            default=managers  # show all by default
        )

        plot_df = timeseries[timeseries["Manager"].isin(selected_mgrs)].copy()
        if plot_df.empty:
            st.info("Select at least one manager to display.")
            return
        ...
        fig = px.line(
            ...
            color="Manager",
            markers=True,
            title=f"{METRIC_LABELS[metric]} — {season}",
        )
        fig.update_layout(
            xaxis_title="Week",
