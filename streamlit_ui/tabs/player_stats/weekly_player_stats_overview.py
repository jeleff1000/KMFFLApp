...
class StreamlitWeeklyPlayerDataViewer:
    ...
    def get_unique_values(self, column, filters):
        ...
        return ["All"] + list(filtered_data[column].unique())
    ...
    def display(self):
        ...
        def display_filters(tab_index):
            ...
            # First row: Player search bar and Owner dropdown
            col1, col2 = st.columns([2, 1])
            with col1:
                player_search = st.text_input("Search Player", key=f"player_search_{tab_index}")
            with col2:
                owner_values = st.multiselect("Select Owner", self.get_unique_values("owner", selected_filters), default=["All"], key=f"owner_value_{tab_index}")
            if player_search:
            ...
            with col1:
                position_values = st.multiselect("Select Position", self.get_unique_values("position", selected_filters), default=["All"], key=f"position_value_{tab_index}")
            with col2:
                fantasy_position_values = st.multiselect("Select Fantasy Position", self.get_unique_values("fantasy position", selected_filters), default=["All"], key=f"fantasy_position_value_{tab_index}")
            ...
            with col1:
                team_values = st.multiselect("Select Team", self.get_unique_values("team", selected_filters), default=["All"], key=f"team_value_{tab_index}")
            with col2:
                opponent_team_values = st.multiselect("Select Opponent Team", self.get_unique_values("opponent_team", selected_filters), default=["All"], key=f"opponent_team_value_{tab_index}")
            with col3:
                week_values = st.multiselect("Select Week", self.get_unique_values("week", selected_filters), default=["All"], key=f"week_value_{tab_index}")
            with col4:
                year_values = st.multiselect("Select Year", self.get_unique_values("season", selected_filters), default=["All"], key=f"year_value_{tab_index}")
        ...
        with tabs[0]:
            ...
            filtered_data = self.apply_filters(filters)
            position = filters.get("position", ["All"])[0]
